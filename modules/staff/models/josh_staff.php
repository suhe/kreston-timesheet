		      ...
		      if (strlen($_FILES['image']['name']))
                {
			         if(!$this->upload->do_upload('image'))
                     {
				        //$this->upload->display_errors();
				        //exit();
						$this->session->set_flashdata('message','<div class=" message success"> This Not Photo File or Your Photo is bigger data !</div>');
    ...
    function updateSet(){
        $data=array('name'      =>  $this->input->xss_clean($_POST['name']),
       	...
       	if($_FILES):
            ...
            $image = $this->upload->data();
                if ($image['file_name']):
                    $data['photo'] = "/assets/photo/".$image['file_name'];
    		    endif;  
            endif;
        endif;
        $this->db->where('no',$this->input->xss_clean($_POST['no']));
