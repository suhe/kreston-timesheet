        <h3 style="padding:10px 20px;border-bottom:1px solid #CCC ">Timecharge Summary Report</h3>
		
		<div id="search" style="width:500px">
		<?=form_open('report/user/timecharge_summary_query','style="margin:0"')?>
		   <?=form_input(array('name' => 'query','value' => $this->session->userdata('timecharge_summary.name'),'placeholder'=>'Search By Client Name','style'=>'padding:7px;widht:200px;border:1px solid #CCC'));?>
		   <?=form_dropdown('status',array(''=>'All','active'=>'Active','deactive'=>'Non Active'),!$this->session->userdata('timecharge_summary.status')?'':$this->session->userdata('timecharge.status'),'style="padding:5px;border:1px solid #CCC"');?>
